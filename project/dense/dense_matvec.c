    ...
    char ** argv)
{
  ...
  double * x = malloc( mat->ncols * sizeof(*x) );
  double * y;
  ...
  zerovec<<<ceil(double(n)/double(THREADSPERBLOCK)), THREADSPERBLOCK>>>(x,mat->nrows);

  matvec<<<ceil(double(n*n)/double(THREADSPERBLOCK)), THREADSPERBLOCK>>> (dev_a, dev_x, dev_y, mat->nrows);
    ...
    int n)
{
  ...
  int global_tid = threadId.y + blockDim.x * blockIdx.x;
...
__global__ void zerovec(
    ...
    int n)
{
  x[ threadId.x + blockDim.x * blockIdx.x ] = 0;
}
